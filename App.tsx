...
const App: React.FC = () => {
  ...
  const handleFileSelect = useCallback(async (file: File) => {
    ...
    setIsConsoleOpen(true); // Auto open console on new process

    addLog(`User selected file: ${file.name}`);

    try {
      const result = await extractFramesFromVideo(file, addLog);
      setFrames(result.frames);
      setMetadata(result.metadata);
      setStatus(ProcessStatus.SUCCESS);
      addLog('SUCCESS: Frames extracted successfully.');
    } catch (err: any) {
      console.error(err);
  ...
  return (
    <div className="min-h-screen bg-slate-50 relative pb-20"> {/* pb-20 for DebugConsole space */}
        ...
        <div className="max-w-4xl mx-auto space-y-8">
          ...
          <div className="text-center space-y-2">
            ...
            <p className="text-lg text-slate-600 max-w-2xl mx-auto">
              Extract the first and last frames locally.
            </p>
          </div>
            ...
            <div className="p-6 sm:p-10">
              ...
              {status === ProcessStatus.PROCESSING && (
                <div className="flex flex-col items-center justify-center py-16 space-y-4 animate-pulse">
                  ...
                  <p className="text-sm text-slate-400">Please wait. Do not switch tabs on mobile.</p>
                </div>
              )}
